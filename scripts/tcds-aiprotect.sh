...
show_recent_cves() {
    ...
    echo "CVEs by Year:"
    # FIX: Replaced the non-portable 'seq' command with a compatible 'while' loop.
    # This resolves the 'seq: not found' error on the router's OS.
    ...
    echo ""
    # FIX: Replaced 'seq' with a compatible 'grep' pattern to count total recent CVEs.
    local total_recent=$(grep -E "CVE-($(echo $(seq $start_year $current_year) | sed 's/ /|/g'))-" "$rules_file" 2>/dev/null | sort -u | wc -l)
    ...
    while true; do
        ...
        case "$choice" in
            ...
            4)
                ...
                search_specific_cve
                ;;
            # FIX: Added 'e' and 'E' as valid exit options for user convenience.
            0|e|E)
                return 0
                ;;
            *)
