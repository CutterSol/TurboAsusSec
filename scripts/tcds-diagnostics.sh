...
run_full_diagnostics() {
    ...
    if skynet_installed; then
        ...
        # FIX: Replaced the unreliable 'command -v' check with a direct check for the executable.
        # This will fix the false 'ipset command not available' warning in the diagnostics output.
        if [ -x "/usr/sbin/ipset" ]; then
            local skynet_ipsets=$(/usr/sbin/ipset list -n 2>/dev/null | grep -i skynet)
            if [ -n "$skynet_ipsets" ]; then
                echo "$skynet_ipsets" | while read ipset_name; do
                    local count=$(/usr/sbin/ipset list "$ipset_name" 2>/dev/null | grep -c "^[0-9]")
