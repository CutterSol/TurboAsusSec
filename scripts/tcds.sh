...
# Instead of looking for a local file, it now re-runs the standard curl command
# to download and execute the latest installer from GitHub, as requested.
run_update() {
    ...
    if curl -sSL https://raw.githubusercontent.com/CutterSol/TurboAsusSec/main/install.sh -o /tmp/tcds-install.sh && sh /tmp/tcds-install.sh; then
        echo ""
        print_info "Update process finished. To use the new version, please exit and re-run 'tcds'."
        # The installer has its own pause, so we don't need another one here.
    else
...
show_main_menu() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    # NOTE: Versioning is now left untouched, as requested. It will display the version from tcds-core.sh.
